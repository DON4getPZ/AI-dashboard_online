...
echo ================================================================================
echo GA4 Data Downloader - Setup Script
echo ================================================================================
echo.
...
REM 기존 config_ga4.json이 있는지 확인
if exist config_ga4.json (
    echo Found existing config_ga4.json
    set /p USE_EXISTING="Use existing configuration? (Y/N): "
    if /i "!USE_EXISTING!"=="Y" (
        echo [OK] Using existing config_ga4.json
        REM config_ga4.json에서 값 읽기
        for /f "usebackq tokens=*" %%i in (`powershell -NoProfile -Command "(Get-Content config_ga4.json | ConvertFrom-Json).google.credentials_path"`) do set GOOGLE_JSON=%%i
        for /f "usebackq tokens=*" %%i in (`powershell -NoProfile -Command "(Get-Content config_ga4.json | ConvertFrom-Json).google.sheet_id"`) do set SHEET_ID=%%i
        for /f "usebackq tokens=*" %%i in (`powershell -NoProfile -Command "(Get-Content config_ga4.json | ConvertFrom-Json).google.worksheet_name"`) do set WORKSHEET_NAME=%%i
...
powershell -Command "$config = @{google=@{credentials_path='%GOOGLE_JSON:\\=\\%';sheet_id='%SHEET_ID%';worksheet_name='%WORKSHEET_NAME%'}}; $config | ConvertTo-Json -Depth 10 | Out-File -Encoding UTF8 config_ga4.json"

if exist config_ga4.json (
    echo [OK] config_ga4.json created
) else (
    echo [ERROR] Failed to create config_ga4.json
    pause
    exit /b 1
)
...
if /i "%RUN_TEST%"=="Y" (
    echo.
    echo ========================================================================
    echo Downloading GA4 Data from Google Sheets
    echo ========================================================================
    ...
    python scripts\fetch_ga4_sheets.py
    ...
    if exist data\GA4 (
        ...
        echo ====================================================================
        echo.
        echo Files saved in: data\GA4\
        dir /b data\GA4\*.csv 2>nul
        echo.
        goto DOWNLOAD_SUCCESS
    ) else (
        echo.
        echo [ERROR] data\GA4 directory not created
        goto DOWNLOAD_FAILED
    )
)
...
echo    python scripts\fetch_ga4_sheets.py
echo.
